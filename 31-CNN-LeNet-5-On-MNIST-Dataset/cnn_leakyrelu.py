...
class LeNet_5_MNIST_Dataset():
    ...
    def __init__(self, img_rows, img_cols, num_classes, dropout_value, optimizer, 
    ...
    def fit_model(self):
        ...
        self.model.add(Conv2D(num_filters_conv_layer1, kernel_size=(filter_size,filter_size), padding='same',
                              kernel_initializer=self.weight_initialization, input_shape=self.input_shape))
        ...
        self.model.add(Conv2D(num_filters_conv_layer2, kernel_size=(filter_size,filter_size), kernel_initializer=self.weight_initialization))
        self.model.add(keras.layers.LeakyReLU())
        self.model.add(MaxPooling2D(pool_size=(max_pool_size,max_pool_size)))
        self.model.add(Dropout(self.dropout_value)) 
        ...
        self.model.add(Dense(num_filters_fc_layer1, kernel_initializer=self.weight_initialization,))
        self.model.add(keras.layers.LeakyReLU())
 
        self.model.add(Dense(num_filters_fc_layer2, kernel_initializer=self.weight_initialization,))
        self.model.add(keras.layers.LeakyReLU())
    ...
    for optmizer in optmizer_list:
        for lr_decay_val in lr_decay_list:
            for weight_initialization in weight_initialization_list:
                ...
                my_LeNet5 = LeNet_5_MNIST_Dataset(img_rows=28, img_cols=28, num_classes=10, dropout_value=dropout_val, optimizer=optmizer,
                ...
                result_df.loc[count,'Activation'] = 'LeakyReLU'
