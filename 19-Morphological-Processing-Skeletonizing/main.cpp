    ...
    for(int i=0; i < ROW; i++){
        for(int j = 0; j < COL; j++){
            ...
            if(image2DBinary[i][j] != 0) {
                ...
                //cout << bondValue << endl;
                int bondSize = returnSkeletonizeBondSize(bondValue);
                //cout << bondSize << endl;
                unsigned char ** conditionalMasks = NULL;
                if (bondSize != 0) {
                    conditionalMasks = returnSkeletonizeConditionalMasks(bondValue);
                    //cout << "----" << endl;
    ...
    for(int i=0; i < ROW; i++){
        for(int j = 0; j < COL; j++){
            ...
            if(image2DInterMed[i][j] != 0) {
                ...
                unConditionalMasks = returnSkeletonizeUnconditionalMasks();
                //cout << "----" << endl;
                int hitOrMiss = compareWithMask(unConditionalMasks, tempPixelArray, MASKSIZE, 26);
                if(hitOrMiss == 0) {
                    image2DOutput[i][j] = 0;
                }
